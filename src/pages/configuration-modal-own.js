...
export default function ConfigurationModalOwn(props) {
	...
	//const [textComponentState, setTextComponentState] = React.useState(props.qts);
  ...
  const handleChange = ({ target }) => {
	...
	const reg = () => {
		...
		if ((stateA.valA === '') || (stateA.valA === undefined)) {
			setError({'color':'red'});
			return;
		}
		
		if ((state8.val6 === '') || (state8.val6 === undefined)) {
			state8.val6 = '-';
		}
	...
	const getTotalprice = () => {
		...
		{state10.val8 && state10.val8.map((custom2, key2) => { 
			var comp = 'text'.concat('||').concat(custom2[0]);
			
			if (!isNaN(textComponentState[comp]) && !isNaN(parseFloat(textComponentState[comp]))) {
				sum = parseFloat(sum) + parseFloat(textComponentState[comp]);
		...
		/*var sum = 0;
		for ( var comp in textComponentState) {
			console.log("Hey!");
			console.log(comp);
			if (!isNaN(textComponentState[comp]) && !isNaN(parseFloat(textComponentState[comp]))) {
				sum = parseFloat(sum) + parseFloat(textComponentState[comp]);
  ...
  <hr></hr>
  <p><b><h8>Color & Quantity:</h8></b></p>
  <font size="2"><p><i>**The colors displayed below are for the design shown for this product. <font color="red"><b>Both In-stock and out of stock items can be ordered.</b></font></i></p></font>
  
  {state10.val8 && state10.val8.map((custom2, key2) => { 
	return (
	<div>
	<Form.Check name={custom2[0]} type="checkbox" label={custom2[0]+ " ("+custom2[2]+")"} onChange={handleToggle} checked={componentState[custom2[0]]}/>
	<b><font size="2" color="blue">Quantity:</font></b>
	<InputGroup className="mb-3" style={styleTEXT}>
    <FormControl
	name={"text".concat("||").concat(custom2[0])}
	onChange={handleChange}
	value = {textComponentState["text".concat("||").concat(custom2[0])]}
      placeholder=""
      aria-label="Recipient's username"
      aria-describedby="basic-addon2"
    />
