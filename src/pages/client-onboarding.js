...
function ClientOnboarding() {
  ...
  const onSave = (componentState, schema_name, sftp, projectName) => {
	var request_map = {}
	...
	request_map['project_name'] = projectName;
	...
	request_map['created_at'] = datetime;
	request_map['last_run_at'] = datetime;
  ...
  const onGetLogs = (componentState, schema_name, sftp) => {
	...
	data: request_map
		...
		axios.get("http://10.166.90.51:8102/complete-logs")
		.then(response2 => {
			//console.log(response)
	...
	intervalID = setInterval(() => axios.get("http://10.166.90.51:8102/logs")
		.then(response => {
			//console.log(response)
...
React.useEffect(() => {
  ...
  <Button variant="info" size="lg" block style={styleB3} onClick={() => onFileDownload()}>
	...
	<div style={{'margin-right':'400px'}}><p><b><font size="3">(Note: If you have already uploaded the files via Yarkon s3, you would just need to specify the s3 folder name above in Configure Global Parameters. No need to upload files in this step.)</font></b></p></div>
...
<div style={{'margin-top': '250px'}}>
	...
	{responseData && responseData['data'].map((custom, key) => {
			  return (
				<Card>
    ...
    <Accordion.Collapse eventKey={custom.module_name}>
	  ...
	  <div>
	  <div style={module2}>
	  <Form.Check name={custom.module_name.concat('||').concat(custom2.app).concat('||').concat(custom2.step_name)} type="checkbox" label={custom2.step_display_name} onChange={handleToggle}/>
	  </div>
	  <div style={configure}>
